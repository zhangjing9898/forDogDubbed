...
exports.find=function *(next) {
    ...
    var queryArray=[
        Creation.find({finish:100})
            .sort({
                'meta.createAt':-1
            })
            .skip(offset)
            .limit(count)
            .populate('author',userFields.join(' '))   //填充content
            .exec(),
        Creation.count({finish:100}).exec()    //简单统计集合中符合某种条件的文档数量
    ]
...
function asyncMedia(videoId,audioId) {
    ...
    Promise.all([
        ...
        var audio_public_id=audio.public_id.replace(/\//g,':')  //把\替换成：
